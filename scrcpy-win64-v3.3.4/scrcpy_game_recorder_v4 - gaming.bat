...
echo [5] CAPCUT DESKTOP MODE (1080p Tablet Mượt & Sắc Nét, DPI 160)
set /p "res_choice=Chon muc (1-5): "

:: Chế độ CapCut tối ưu hóa: Sắc nét nhờ H.265 nhưng mượt mà nhờ độ phân giải 1080p vừa phải
if "%res_choice%"=="5" (
    set "new_res=1920x1080/160"
    set "v_br=20M"
    set "ADDITIONAL_ARGS=--video-codec=h265"
    echo [*] Da thiet lap che do: CAPCUT SMOOTH-SHARP (1920x1080@160DPI - H.265)
    set "search_term=lemon"
    goto :app_search_preset
)
...
if %count%==0 (
    echo [!] Khong tim thay App voi tu khoa "%search_term%".
    goto :app_search
)
...
:: --- KHOI CHAY SCRCPY ---
set "COMMON_ARGS=--new-display=%new_res% --start-app=+%target_pkg% --video-bit-rate %v_br% --video-buffer=0 --stay-awake --audio-source=output --turn-screen-off !G_ARGS! %ADDITIONAL_ARGS%"
...
echo ------------------------------------------------------
if "%game_mode%"=="2" echo Che do Gaming FPS dang bat.